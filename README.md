...
To start all related Service Manager services from the latest RELEASE version instead of snapshot execute the following:

```
sm2 --start ITVC_FINANCIALS_TEST --appendArgs '{"CITIZEN_DETAILS":["-Dmongodb.cid-sautr-cache.enabled=false"]}'
```
...
```
sbt "run 9092 -Dplay.http.router=testOnlyDoNotUseInAppConf.Routes"
```
