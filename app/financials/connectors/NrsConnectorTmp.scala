...
@Singleton
class NrsConnectorTmp @Inject()(http: HttpClientV2, appConfig: FrontendAppConfig)(
  ...
  def invalidSubmit(remainingAttempts: Int = numberOfRetries): Future[NrsSubmissionResponse] = {
    ...
    http
      ...
      .flatMap {
        ...
        case response if shouldRetry(response) && remainingAttempts > 0 =>
          ...
          invalidSubmit(remainingAttempts = remainingAttempts - 1)
